...
public class UITutorial : MonoBehaviour
{
    ...
    {
        if (TutorialManager.instance._teamCTWin > TutorialManager.instance._teamTerroristWin)
        {
        ...
        else if (TutorialManager.instance._teamCTWin < TutorialManager.instance._teamTerroristWin)
        {
            for (int i = 0; i < _resultTerrorist.Length; i++)
            {
    ...
    {
        TutorialManager.instance.BuyWeapon(_currentWeaponIndex, TutorialManager.instance._playerController, TutorialManager.instance._playerInventory, TutorialManager.instance._playerHealth);
        AudioManager.instance.PlaySfx(SFXType.MetalClick);
    ...
    public void UpdateUITime(float timeCount)
    {
        ...
        if (TutorialManager.instance._currentGameState == GameState.Countdown)
        {
            _time.text = $"<color=#FF0000>{minutes:00} : {seconds:00}</color>";
        }
        else if (TutorialManager.instance._currentGameState == GameState.RoundActive)
        {
    ...
    {
        if (playerHealth == TutorialManager.instance._playerHealth)
